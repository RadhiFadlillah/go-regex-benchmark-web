...
- For some reasons code that generated by re2go has inconsistent performance. For the shortest pattern (email) it's slower than Go's standard library. However for the longest pattern (IP) it has similar performance as RE2 with cgo binding.
